...
# -----------------------------------------------------------------------------
# Get neighborhood discovery errors from individual nodes rx analysis of each
# simulation and put in a global file to each node
#
# May 22, 2023
#
# Run from inside flysafe traces folder
#			 (e.g. ns3.34/flysafe_traces# ./_globalNeighDiscoveryErrorData.sh)
#
# Execution order: 2
# -----------------------------------------------------------------------------
...
do
	IPFile="neighborhood_rx_statistics_192.168.1."$i".txt"
	find . -name $IPFile  > _traceRxFiles # get nodes files from all simulations
	IPFileGlobal="global_neighborhood_rx_statistics_192.168.1."$i".txt"
	echo -e "dateTime\tmin\tmax\tmean\tstddev" >> $folder"/"$IPFileGlobal # insert a header line in global file
